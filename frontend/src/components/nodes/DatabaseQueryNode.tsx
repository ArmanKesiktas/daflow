...
function DataPreviewPopup({ data, pos }: { data: PreviewData; pos: { top: number; left: number } }) {
  ...
  return createPortal(
    <div
      ...
      className="w-[520px] bg-[#ffffff] dark:bg-[#1C1C1E] rounded-2xl border border-[var(--color-border-default)] shadow-2xl p-4 pointer-events-none -translate-y-1/2"
    >
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-[12px] font-semibold text-[var(--color-text-primary)]">Query Result Preview</h4>
      ...
      <div className="mb-3">
        <div className="text-[10px] font-semibold text-[var(--color-text-muted)] uppercase tracking-wider mb-1.5">Schema</div>
        <div className="overflow-auto max-h-24">
          <table className="w-full text-[10px]">
            <thead>
              <tr className="border-b border-[var(--color-border-default)]">
                <th className="text-left pb-1 font-medium text-[var(--color-text-muted)]">Column</th>
                <th className="text-left pb-1 font-medium text-[var(--color-text-muted)]">Type</th>
                <th className="text-right pb-1 font-medium text-[var(--color-text-muted)]">Missing</th>
                <th className="text-right pb-1 font-medium text-[var(--color-text-muted)]">Unique</th>
              </tr>
            </thead>
            <tbody>
              {data.metadata.columns.slice(0, 10).map((col) => (
                <tr key={col.name} className="border-b border-[var(--color-border-subtle)]">
                  <td className="py-0.5 text-[var(--color-text-primary)] truncate max-w-[140px]">{col.name}</td>
                  <td className="py-0.5 text-[var(--color-text-muted)] font-mono">{col.dtype}</td>
                  <td className="py-0.5 text-right text-[var(--color-text-muted)]">{col.missing_count}</td>
                  <td className="py-0.5 text-right text-[var(--color-text-muted)]">{col.unique_count}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="overflow-auto max-h-28">
            <table className="w-full text-[10px]">
              <thead>
                <tr className="border-b border-[var(--color-border-default)]">
                  {cols.map((c) => (
                    <th key={c.name} className="text-left pb-1 font-medium text-[var(--color-text-muted)] truncate max-w-[80px] pr-2">{c.name}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} className="border-b border-[var(--color-border-subtle)]">
                    {cols.map((c) => (
                      <td key={c.name} className="py-0.5 text-[var(--color-text-secondary)] truncate max-w-[80px] pr-2">{row[c.name] ?? ''}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
...
export const DatabaseQueryNode = memo(function DatabaseQueryNode({ id, data, selected }: NodeProps<Node<NodeData>>) {
  ...
  const dbColor = DB_COLORS[dbType] ?? '#6B7280'
  ...
  return (
    <div ref={nodeRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <BaseNode
        ...
        icon={dbType.charAt(0).toUpperCase()}
        status={data.status}
        color={dbColor}
      ...
      >
        <span className="flex items-center gap-1.5">
          ...
          {useSSH && <span className="text-[10px] bg-warning/15 text-warning px-1.5 py-0.5 rounded-md font-medium">SSH</span>}
        </span>
        {queryPreview && (
          <span className="font-mono text-[10px] text-[var(--color-text-muted)] truncate">{queryPreview}</span>
        )}
        {data.status === 'success' && (
